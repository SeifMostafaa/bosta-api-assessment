const { checkService } = require('.');
const { ReportModel, CheckModel } = require('../models');
...
const sendStatusEmails = async (url, status, users) => new Promise(() => {
  const promises = users?.map((user) => send({
    to: user.email,
...
module.exports = {
  initReport, changeReportsStatus, getReport, sendStatusEmails, sendNotifications,
};
